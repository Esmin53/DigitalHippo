                        ...
                        {isMounted && items.length === 0 ? (
                            <div className="flex h-full flex-col items-center justify-center space-y-1">
                                <div aria-hidden='true' className="relative mb-4 h-40 w-40 text-muted-foreground">
                                    <Image src={'/hippo-empty-cart.png'} loading="eager" fill alt="Empty shopping cart hippo" />
                                </div>
                                <h3 className="font-semibold text-2xl">Your cart is empty</h3>
                                <p className="text-muted-foreground text-center">Whoops! Nothing to show here.</p>
                            </div>
                        ) : null}
                                ...
                                return (<li key={product.id} className="flex py-6 sm:py-10">
                                    <div className="flex-shrink-0">
                                        <div className="relative h-24 w-24">
                                            {typeof image !== 'string' && image.url ? (
                                                <Image src={image.url} alt="Product image" fill className="h-full w-full rounded-md object-cover object-center sm:h-48 sm:w-48"/>
                                            ) : null}
                                        </div>
                                    </div>
                                    ...
                                    <div className="ml-4 flex flex-1 flex-col justify-betweensm:ml-6">
                                        <div className="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
                                            <div>
                                                <div className="flex justify-between">
                                                    <h3 className="text-sm">
                                                        <Link href={`/product/${product.id}`} className="font-md text-gray-700 hover:text-gray-800">
                                                            {product.name}
                                                        </Link>
                                                    </h3>
                                                </div>
                                            ...
                                            <div className="mt-4 sm:mt-0 sm:pr-9 w-20">
                                                <div className="absolute right-0 top-0">
                                                    <Button aria-label="Remove item" onClick={() => removeItem(product.id)} variant='ghost'>
                                        ...
                                        <p className="mt-4 flex space-x-2 text-sm text-gray-700">
                                            ...
                                            <span>Eligable for instant delivery</span>
                                        </p>
                                    </div>
                                </li>)
                            })}
                    ...
                    <section className="mt-16 rounded-lg bg-gray-50 px-4 py-6 sm:p-6 lg:col-span-5 lg:mt-0 lg:p-8">
                        ...
                        <div className="mt-6 space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <p className="text-sm font-md text-gray-900">{isMounted ? formatPrice(cartTotal) : 
                                <Loader2 className="h-4 w-4 animate-spin text-muted-foreground"/>}</p>
                            </div>
                        ...
                        <div className="mt-6">
                            <Button className="w-full" size='lg' onClick={() => createCheckoutSession({productIds})}
                            disabled={isLoading || items.length === 0}>
                                {isLoading ? (<Loader2 className="h-4 w-4 animate-spin" />) : 'Checkout'}
                            </Button>
                        </div>
                    </section>
                </div>
